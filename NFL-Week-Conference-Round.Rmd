---
title: "NFL Divisional Round, 2019"
...
week_11 = read_csv("NFL Week 11.csv")
week_12 = read_csv("NFL Week 12.csv")
week_13 = read_csv("NFL Week 13.csv")
week_14 = read_csv("NFL Week 14.csv")
week_15 = read_csv("NFL Week 15.csv")
week_16 = read_csv("NFL Week 16.csv")
week_17 = read_csv("NFL Week 17.csv") 
week_18 = read_csv("NFL Wild Card.csv") 
week_19 = read_csv("NFL Divisional Round.csv") 
data = read_csv("NFL Conference Round.csv") #Update
cadet_wk1 = read_csv("NFL Week 1 MA206.csv")
...
cadet_wk10 = read_csv("NFL Week 10 MA206.csv")
cadet_wk11 = read_csv("NFL Week 11 MA206.csv")
cadet_wk12 = read_csv("NFL Week 12 MA206.csv")
cadet_wk13 = read_csv("NFL Week 13 MA206.csv") 
cadet_wk14 = read_csv("NFL Week 14 MA206.csv") 
cadet_wk15 = read_csv("NFL Week 15 MA206.csv") 
cadet_wk16 = read_csv("NFL Week 16 MA206.csv") 
c_data = read_csv("NFL Week 17 MA206.csv") # Update
```

```{r games, include=FALSE}
#basic info

n = 2 #number of games this week
...
wk17_n = 16 # number of games in week 17
wk18_n = 4 # number of games in week 18
wk19_n = 4 # number of games in week 19
wk_max = c(wk1_n, wk2_n, wk3_n, wk4_n, wk5_n, wk6_n, wk7_n, wk8_n, wk9_n, wk10_n, wk11_n, wk12_n, wk13_n, 
           wk14_n, wk15_n,wk16_n,wk17_n, wk18_n, wk19_n, n)
...
g16_n = 14 # number of group picks in week 16
g17_n = 16 # number of group picks in week 17
g18_n = 4 # number of group picks in week 18
g19_n = 2 # number of group picks in week 19
...
wk16_wins = 10 #number of wins in week 16 
wk17_wins = 8 #number of wins in week 17 
wk18_wins = 3 #number of wins in week 18 
wk19_wins = 1 #number of wins in week 18 #update this
...
wk16_combined_wins = 14 # number of combined wins in week 16 
wk17_combined_wins = 8 # number of combined wins in week 17
wk18_combined_wins = 3 # number of combined wins in week 18 
wk18_combined_wins = 1 # number of combined wins in week 19 # update this

# who actually won
winners = c("Kansas City Chiefs","San Francisco 49ers")

CBSsports = c("Kansas City Chiefs","San Francisco 49ers")
...
wk17_winners = c("Cincinnatti Bengals","Miami Dolphins","Chicago Bears","Kansas City Chiefs","New York Jets","Green Bay Packers","New Orleans Saints","Atlanta Falcons","Philadelphia Eagles","Tennessee Titans","Dallas Cowboys","Baltimore Ravens","Jacksonville Jaguars","Denver Broncos","Los Angeles Rams","San Francisco 49ers")

wk18_winners = c("Houston Texans","Tennessee Titans","Minnesota Vikings","Seattle Seahawks")

wk19_winners = c("San Francisco 49ers","Tennessee Titans","Kansas City Chiefs","Green Bay Packers")
...
#wk17_CBSsports = c("Cleveland Browns","New England Patriots","Minnesota Vikings","Kansas City Chiefs","New York Jets","Green Bay Packers","New Orleans Saints","Atlanta Falcons","Philadelphia Eagles","Tennessee Titans","Dallas Cowboys","Pittsburgh Steelers","Indianapolis Colts","Denver Broncos","Arizona Cardinals","Seattle Seahawks")

#wk18_CBSsports = c("Buffalo Bills","New England Patriots","New Orleans Saints","Philadelphia Eagles")

#wk19_CBSsports = c("San Francisco 49ers","Baltimore Ravens","Kansas City Chiefs","Green Bay Packers")
```
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n+g9_n+g10_n+g11_n+g12_n+
  g13_n+g14_n+g15_n+g16_n+g17_n+g18_n+g19_n #Update
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+wk11_wins+wk12_wins+wk13_wins+wk14_wins+wk15_wins+wk16_wins+wk17_wins+wk18_wins+wk19_wins+
  correct_count$n[which(correct_count$Correct == "Yes")] #Update
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               ...
               (wk14_wins/g14_n),(wk15_wins/g15_n),(wk16_wins/g16_n),(wk17_wins/g17_n),
               (wk18_wins/g18_n),(wk19_wins/g19_n),weekly_win_percentage),4) #Update
...
season = indiv_weekly_pred(week_1, wk1_winners) %>%
  ...
  full_join(indiv_weekly_pred(week_16, wk16_winners) %>%
      rename("Week_16" = Week)) %>%
  full_join(indiv_weekly_pred(week_17, wk17_winners) %>%
      rename("Week_17" = Week)) %>%
  full_join(indiv_weekly_pred(week_18, wk18_winners) %>%
      rename("Week_18" = Week)) %>%
  full_join(indiv_weekly_pred(week_19, wk19_winners) %>%
      rename("Week_19" = Week)) %>%
  full_join(indiv_weekly_pred(data, winners) %>% ####### Add line for previous week and update to current week
      rename("Week_20" = Week)) %>%
  mutate(Percent = round(Week_20/n,4)) %>%
  arrange(-Percent)
...
current_indiv = formattable(indiv_disp,
            align =c("l","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c"), 
            list(
              `Indicator Name` = formatter("span", 
                                style = ~ style(color = "grey",font.weight = "bold")),
...
indi = indiv_disp %>% 
  ...
  mutate(Week_16 = Week_16/(wk16_n))%>%
  mutate(Week_17 = Week_17/(wk17_n))%>%
  mutate(Week_18 = Week_18/(wk18_n))%>%
  mutate(Week_19 = Week_19/(wk19_n))%>%#Update
  as.data.frame()
na = rep(indi$name, 20) # Update
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10,indi$Week_11,indi$Week_12,indi$Week_13,indi$Week_14,indi$Week_15,indi$Week_16,indi$Week_17,indi$Week_18,indi$Week_19, indi$Percent) # Update
we2 = c(rep("Week 1", length(indi$name)), rep("Week 2", length(indi$name)),
        ...
        rep("Week_17", length(indi$name)),rep("Week_18", length(indi$name)),
        rep("Week_19", length(indi$name)),rep("Week_20", length(indi$name))) #Update
