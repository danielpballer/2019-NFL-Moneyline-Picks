...
data = read_csv("NFL Wild Card.csv") #Update
...
wk17_wins = 8 #number of wins in week 17 #update this
...
wk17_combined_wins = 8 # number of combined wins in week 17 # update this

# who actually won
winners = c("Houston Texans","Tennessee Titans","Minnesota Vikings","Seattle Seahawks")


CBSsports = c("Buffalo Bills","New England Patriots","New Orleans Saints","Philadelphia Eagles")
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n+g9_n+g10_n+g11_n+g12_n+
  g13_n+g14_n+g15_n+g16_n+g17_n #Update
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+wk11_wins+wk12_wins+wk13_wins+wk14_wins+wk15_wins+wk16_wins+wk17_wins+
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               ...
               (wk14_wins/g14_n),(wk15_wins/g15_n),(wk16_wins/g16_n),(wk17_wins/g17_n), weekly_win_percentage),4) #Update
weeks = c("Week 1", "Week 2", "Week 3", "Week 4", "Week 5", "Week 6", "Week 7", "Week 8", "Week 9",
          "Week_10", "Week_11", "Week_12", "Week_13", "Week_14", "Week_15", "Week_16", "Week_17", "Week_18") #Update
...
season = indiv_weekly_pred(week_1, wk1_winners) %>%
  ...
  full_join(indiv_weekly_pred(data, winners) %>% ####### Add line for previous week and update to current week
      rename("Week_18" = Week)) %>%
  mutate(Percent = round(Week_18/n,4)) %>%
  arrange(-Percent)
...
current_indiv = formattable(indiv_disp,
            align =c("l","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c"), 
            list(
              `Indicator Name` = formatter("span", 
                                style = ~ style(color = "grey",font.weight = "bold")),
...
indi = indiv_disp %>% 
  ...
  mutate(Week_17 = Week_17/(wk17_n))%>%#Update
  as.data.frame()
na = rep(indi$name, 18) # Update
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10,indi$Week_11,indi$Week_12,indi$Week_13,indi$Week_14,indi$Week_15,indi$Week_16,indi$Week_17, indi$Percent) # Update
we2 = c(rep("Week 1", length(indi$name)), rep("Week 2", length(indi$name)),
        ...
        rep("Week_17", length(indi$name)),rep("Week_18", length(indi$name))) #Update
