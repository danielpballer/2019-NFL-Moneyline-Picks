...
ind_result = function(x){
  indiv_wk = x %>% 
    select(Name, starts_with("Game"))
  
  games = indiv_wk %>% select(starts_with("Game"))
  
  indiv_correct = NULL
  help = NULL
  for (i in 1:length(indiv_wk$Name)){
    for(j in 1:length(games)){
    help[j] = ifelse(games[i,j]==wk1_winners[j],1,0)
  ...
  season_wk = indiv_wk %>%
  select(Name) %>%
  add_column(Week_1 = indiv_correct)
  return(season_wk)
}

  w = 1
  paste0("wk",w,"_winners") %>% class()
  ?paste0()
season_wk = week_1 %>% ind_result()
