...
library(formattable)
library(ggpubr)
data = read_csv("../CSV_Data_Files/NFL Week 1 MA206.csv")
...
paste("Correct pick percentage for the week is", weekly_win_percentage, sep = " ")
...
paste("Season correct pick percentage so far is", season_win_percentage, sep = " ")
Total = dim(data)[1]
...
indiv_disp = indiv %>%
  select(name,hour, WK1_Correct_Picks, WK1_Percentage)
...
class = indiv_disp %>% 
    group_by(hour) %>%
    summarise(Class_Avg = mean(WK1_Percentage))
