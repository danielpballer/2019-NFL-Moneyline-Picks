...
data = read_csv("../CSV_Data_Files/NFL Week 5.csv")
...
indiv_disp = indiv %>%
  ...
  select(name, Week_1, Week_2, Week_3, Week_4, Week_5, Percent, Weeks_Picked, Season_Percent) %>%
...
c_games = cadet_wk5 %>% janitor::clean_names() %>%
  select(starts_with("game"))  #Creating the set of just games
...
tot_pickers = dim(data)[1] + dim(cadet_wk5)[1]
paste(tot_pickers, "total predictions this week", sep = " ")

c_wins = c((wk1_combined_wins/wk1_n),(wk2_combined_wins/wk2_n),(wk3_combined_wins/wk3_n),
           round((wk4_combined_wins/wk4_n),4), t_weekly_win_percentage)
