...
# week_15 = read_csv("NFL Week 15.csv") # Update
# data = read_csv("NFL Week 16.csv") # Update
...
week_15 = read_csv("../CSV_Data_Files/NFL Week 15 MA206.csv") 
data = read_csv("../CSV_Data_Files/NFL Week 16 MA206.csv") # Update
```

```{r games, include=FALSE}
#basic info

n = 10 #number of games this week
...
wk_max = c(wk1_n, wk2_n, wk3_n, wk4_n, wk5_n, wk6_n, wk7_n, wk8_n, wk9_n, wk10_n, wk11_n, wk12_n, wk13_n, wk14_n,wk15_n, n)
...
g15_n = 16 # number of group picks in week 15
...
wk14_wins = 9 #number of wins in week 14 
wk15_wins = 12 #number of wins in week 15 #update this
...
wk15_combined_wins = 12 # number of combined wins in week 15 # update this

# who actually won
winners = c("Houston Texans","New England Patriots","San Francisco 49ers","Indianapolis Colts","Baltimore Ravens","Atlanta Falcons","New Orleans Saints","Oakland Raiders","Denver Broncos","Green Bay Packers")

CBSsports = c("Tampa Bay Buccaneers","Buffalo Bills","San Francisco 49ers","Indianapolis Colts","Baltimore Ravens","Atlanta Falcons","New Orleans Saints","Los Angeles Chargers","Denver Broncos","Green Bay Packers")
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n+g9_n+g10_n+g11_n+g12_n+
  g13_n+g14_n+g15_n #Update
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+wk11_wins+wk12_wins+wk13_wins+wk14_wins+wk15_wins+correct_count$n[which(correct_count$Correct == "Yes")] #Update
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               ...
               (wk14_wins/g14_n),(wk15_wins/g15_n), weekly_win_percentage),4) #Update
weeks = c("Week 1", "Week 2", "Week 3", "Week 4", "Week 5", "Week 6", "Week 7", "Week 8", "Week 9",
          "Week_10", "Week_11", "Week_12", "Week_13", "Week_14", "Week_15", "Week_16") #Update
...
season = indiv_weekly_pred(week_1, wk1_winners) %>%
  ...
  full_join(indiv_weekly_pred(data, winners) %>% ####### Add line for previous week and update to current week
      rename("Week_16" = Week)) %>%
  mutate(Percent = round(Week_16/n,4)) %>%
  arrange(-Percent)
...
indi = indiv_disp %>% 
  ...
  mutate(Week_15 = Week_15/(wk15_n))%>%#Update
  as.data.frame()
na = rep(indi$name, 16) # Update
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10,indi$Week_11,indi$Week_12,indi$Week_13,indi$Week_14,indi$Week_15, indi$Percent) # Update
we2 = c(rep("Week 1", length(indi$name)), rep("Week 2", length(indi$name)),
        ...
        rep("Week_15", length(indi$name)),rep("Week_16", length(indi$name))) #Update
