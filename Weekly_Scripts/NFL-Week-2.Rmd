...
data = read_csv("../CSV_Data_Files/NFL Week 2.csv")
...
indiv_disp = indiv %>%
  ...
  select(name, Week_1, Week_2, Percent, Weeks_Picked, Season_Percent) %>%
...
c_games = cadet_wk2 %>% janitor::clean_names() %>%
  select(starts_with("game"))  #Creating the set of just games
...
c_wins = c((wk1_combined_wins/wk1_n), t_weekly_win_percentage)
