...
#data = read_csv("NFL Week 2.csv")
week_1 = read_csv("../CSV_Data_Files/NFL Week 1 MA206.csv")
data = read_csv("../CSV_Data_Files/NFL Week 2 MA206.csv")
```
...
wk1_wins = 11
wk1_combined_wins = 12

season_games = n+wk1_n
season_wins  = wk1_wins
# who actually won
winners = c("Tampa Bay Buccaneers","Baltimore Ravens","Dallas Cowboys","Indianapolis Colts","Seattle Seahawks","Buffalo Bills","San Fransisco 49ers","Detroit Lions","Green Bay Packers","Houston Texans","New England Patriots","Kansas City Chiefs","Los Angeles Rams","Chicago Bears","Atlanta Falcons","Cleveland Browns")

CBSsports = c("Carolina Panthers","Baltimore Ravens","Dallas Cowboys","Indianapolis Colts","Seattle Seahawks","Buffalo Bills","Cincinnati Bengals","Los Angeles Chargers","Green Bay Packers","Houston Texans","New England Patriots","Kansas City Chiefs","Los Angeles Rams","Chicago Bears","Philadelphia Eagles","Cleveland Browns")
...
season_win_percentage = (correct_count$n[which(correct_count$Correct == "Yes")]+season_wins)/(season_games)
...
wins = c((wk1_wins/wk1_n), weekly_win_percentage)
...
indiv = indiv %>% 
  add_column(WK2_Correct = indiv_correct) %>%
  mutate(WK2_Percent = WK2_Correct/n) %>%
  rename("Week_2" = WK2_Correct)%>%
  rename("Percent" = WK2_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name, hour, Week_2, Percent) %>%
  full_join(season) %>%
  mutate(w1p = ifelse(is.na(Week_1)==T,0,1)) %>%
  mutate(w2p = ifelse(is.na(Week_2)==T,0,1)) %>%
  mutate(Weeks_Picked = w1p+w2p) %>%
  mutate(games_picked = ((w1p*16)+(w2p*16))) %>%
  mutate(Season_Percent = (ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ifelse(is.na(Week_2)==T, 0, Week_2))/games_picked) %>%
  select(name, hour, Week_1, Week_2, Percent, Weeks_Picked, Season_Percent) %>%
...
Week_2_indiv = formattable(indiv_disp,
            align =c("l","c","c", "c", "c"), 
            list(
              `Indicator Name` = formatter("span", 
                                style = ~ style(color = "grey",font.weight = "bold")),
...
indi = indiv_disp %>% 
  select(name, Week_1, Percent) %>%
  mutate(Week_1 = Week_1/(wk1_n))%>%
  as.data.frame()
na = rep(indi$name, 2)
we = c(indi$Week_1, indi$Percent)
wee = c(rep("Week 1", length(indi$name)), rep("Week 2", length(indi$name)))
i_df = cbind.data.frame(na, we, wee)

ggplot(i_df, aes(x = wee, y = we, group = na, color = na))+
  geom_point()+
  geom_line()+
