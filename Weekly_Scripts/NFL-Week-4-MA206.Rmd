...
data = read_csv("../CSV_Data_Files/NFL Week 4 MA206.csv")
```
...
wk3_wins = 10 #number of wins in week 3 #update this
wk1_combined_wins = 12 # number of combined wins in week 1
wk2_combined_wins = 10 # number of combined wins in week 2
wk3_combined_wins = 10 # number of combined wins in week 3 # update this

season_games = n+wk1_n+wk2_n+wk3_n
season_wins  = wk1_wins+wk2_wins+wk3_wins
# who actually won
winners = c("Philadelphia Eagles","Tennessee Titans","New York Giants","Los Angeles Chargers","Oakland Raiders","Carolina Panthers","Kansas City Chiefs","Cleveland Browns","New England Patriots","Tampa Bay Buccaneers","Seattle Seahawks","Chicago Bears","Jacksonville Jaguars","New Orleans Saints","Pittsburgh Steelers")

CBSsports = c("Green Bay Packers","Atlanta Falcons","Washington Redskins","Los Angeles Chargers","Indianapolis Colts","Carolina Panthers","Kansas City Chiefs","Baltimore Ravens","New England Patriots","Los Angeles Rams","Seattle Seahawks","Chicago Bears","Jacksonville Jaguars","Dallas Cowboys","Pittsburgh Steelers")
...
weekly_win_percentage = round(correct_count$n[which(correct_count$Correct == "Yes")]/n,4)
...
#Previous Weeks
wins = c((wk1_wins/wk1_n),(wk2_wins/wk2_n),(wk3_wins/wk3_n), weekly_win_percentage)
...
indiv = indiv %>% 
  add_column(WK4_Correct = indiv_correct) %>%
  mutate(WK4_Percent = round(WK4_Correct/n,4)) %>%
  rename("Week_4" = WK4_Correct)%>%
  rename("Percent" = WK4_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name, hour, Week_4, Percent) %>%
  full_join(season) %>%
  ...
  mutate(Weeks_Picked = w1p+w2p+w3p+w4p) %>%
  mutate(games_picked = ((w1p*wk1_n)+(w2p*wk2_n)+(w3p*wk3_n)+(w4p*n))) %>%
  mutate(Season_Percent = round(((ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ifelse(is.na(Week_2)==T,0,Week_2) +
                             ifelse(is.na(Week_3)==T, 0, Week_3)+
                             ifelse(is.na(Week_4)==T,0,Week_4))/games_picked),4)) %>%
  select(name, hour, Week_1, Week_2, Week_3, Week_4, Percent, Weeks_Picked, Season_Percent) %>%
...
Week_4_indiv = formattable(indiv_disp,
...
indi = indiv_disp %>% 
  select(name, Week_1, Week_2, Week_3, Percent) %>%
  mutate(Week_1 = Week_1/(wk1_n))%>%
  mutate(Week_2 = Week_2/(wk2_n))%>%
  mutate(Week_3 = Week_3/(wk3_n))%>%
  as.data.frame()
na = rep(indi$name, 4)
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3, indi$Percent)
