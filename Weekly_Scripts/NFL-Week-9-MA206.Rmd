...
data = read_csv("../CSV_Data_Files/NFL Week 9 MA206.csv") # Update
```
...
wk8_wins = 13 #number of wins in week 8 #update this
...
wk8_combined_wins = 13 # number of combined wins in week 8 # update this
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+correct_count$n[which(correct_count$Correct == "Yes")]
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               (wk6_wins/g6_n),(wk7_wins/g7_n),(wk8_wins/g8_n), weekly_win_percentage),4)
...
indiv = data %>% janitor::clean_names() %>%
  select(name, hour, starts_with("game"))
...
indiv = indiv %>% 
  add_column(WK9_Correct = indiv_correct) %>%
  mutate(WK9_Percent = round(WK9_Correct/n,4)) %>%
  rename("Week_9" = WK9_Correct)%>%
  rename("Percent" = WK9_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name, hour, Week_9, Percent) %>%
  full_join(season) %>%
  ...
  mutate(Weeks_Picked = w1p+w2p+w3p+w4p+w5p+w6p+w7p+w8p+w9p) %>%
  mutate(games_picked = ((w1p*wk1_n)+(w2p*wk2_n)+(w3p*wk3_n)+(w4p*wk4_n)+(w5p*wk5_n)+
                           (w6p*wk6_n)+(w7p*wk7_n)+(w8p*wk8_n)+(w9p*n))) %>%
  mutate(Season_Percent = round(((ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ...
                             ifelse(is.na(Week_9)==T,0,Week_9))/games_picked),4)) %>%
  select(name, hour, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Week_7, Week_8, Week_9, 
...
Week_9_indiv = formattable(indiv_disp,
...
class = class[-4,]
```
`r paste("Class correct percentage for the week is G1:", round(class[1,2],4),"G2:", round(class[2,2],4),"I1:", round(class[3,2],4), sep = " ") `

`r paste("Best Prediction this week is hour", class$hour[which(class$Class_Avg==max(class$Class_Avg))]) `

```{r individual graph, eval=FALSE, include=FALSE}
indi = indiv_disp %>% 
  select(name, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Week_7, Week_8, Percent) %>%
  ...
  as.data.frame()
na = rep(indi$name, 9)
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Percent)
