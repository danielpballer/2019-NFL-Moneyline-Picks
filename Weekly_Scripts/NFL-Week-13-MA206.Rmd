...
# data = read_csv("NFL Week 13.csv") # Update
...
week_12 = read_csv("../CSV_Data_Files/NFL Week 12 MA206.csv") # Update
data = read_csv("../CSV_Data_Files/NFL Week 13 MA206.csv") # Update
```
...
wk_max = c(wk1_n, wk2_n, wk3_n, wk4_n, wk5_n, wk6_n, wk7_n, wk8_n, wk9_n, wk10_n, wk11_n, wk12_n, n)
...
wk12_wins = 11 #number of wins in week 12 #update this
...
wk12_combined_wins = 11 # number of combined wins in week 12 # update this

# who actually won
winners = c("Chicago Bears","Buffalo Bills","New Orleans Saints","Tennessee Titans","Cincinnati Bengals","Washington Redskins","Baltimore Ravens","Tampa Bay Buccaneers","Pittsburgh Steelers","Green Bay Packers","Miami Dolphins","Los Angeles Rams","Denver Broncos","Kansas City Chiefs","Houston Texans","Seattle Seahawks")

CBSsports = c("Chicago Bears","Buffalo Bills","New Orleans Saints","Indianapolis Colts","New York Jets","Carolina Panthers","Baltimore Ravens","Tampa Bay Buccaneers","Cleveland Browns","Green Bay Packers","Philadelphia Eagles","Arizona Cardinals","Los Angeles Chargers","Kansas City Chiefs","New England Patriots","Seattle Seahawks")
...
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+wk11_wins+wk12_wins+correct_count$n[which(correct_count$Correct == "Yes")]
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               ...
               (wk10_wins/g10_n),(wk11_wins/g11_n),(wk12_wins/g12_n),  weekly_win_percentage),4)
...
season = indiv_weekly_pred(week_1, wk1_winners) %>%
  ...
  full_join(indiv_weekly_pred(data, winners) %>% ####### Add line for previous week and update to current week
      rename("Week_13" = Week)) %>%
  mutate(Percent = round(Week_13/n,4)) %>%
  arrange(-Percent)
...
current_indiv = formattable(indiv_disp,
            align =c("l","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c","c"), 
            list(
              `Indicator Name` = formatter("span", 
                                style = ~ style(color = "grey",font.weight = "bold")),
...
indi = indiv_disp %>% 
  ...
  as.data.frame()
na = rep(indi$name, 13)
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10,indi$Week_11,indi$Week_12, indi$Percent)
