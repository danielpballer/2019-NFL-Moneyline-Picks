...
data = read_csv("../CSV_Data_Files/NFL Week 5 MA206.csv")
```
...
wk3_wins = 10 #number of wins in week 3
wk4_wins = 8 #number of wins in week 4 #update this
...
wk4_combined_wins = 8 # number of combined wins in week 4 # update this

# who actually won
winners = c("Seattle Seahawks","Carolina Panthers","New England Patriots","Buffalo Bills","Baltimore Ravens","Arizona Cardinals","Houston Texans","New Orleans Saints","Minnesota Vikings","Oakland Raiders","Philadelphia Eagles","Denver Broncos","Green Bay Packers","Indianapolis Colts","San Francisco 49ers")

CBSsports = c("Seattle Seahawks","Jacksonville Jaguars","New England Patriots","Tennessee Titans","Baltimore Ravens","Cincinnati Bangals","Houston Texans","Tamps Bay Buccaneers","Minnesota Vikings","Chicago Bears","Philadelphia Eagles","Los Angeles Chargers","Dallas Cowboys","Kansas City Chiefs","Cleveland Browns")
...
wk4_winners = c("Philadelphia Eagles","Tennessee Titans","New York Giants","Los Angeles Chargers","Oakland Raiders","Carolina Panthers","Kansas City Chiefs","Cleveland Browns","New England Patriots","Tampa Bay Buccaneers","Seattle Seahawks","Chicago Bears","Jacksonville Jaguars","New Orleans Saints","Pittsburgh Steelers") #update this
...
correct_count = count(results, Correct)
 ...
 ifelse(identical(correct_count$n[which(correct_count$Correct == "--")], integer(0))==TRUE, 0, correct_count$n[which(correct_count$Correct == "--")])
...
season_games = group_games+wk1_n+wk2_n+wk3_n+wk4_n
...
#Previous Weeks
wins = round(c((wk1_wins/wk1_n),(wk2_wins/wk2_n),(wk3_wins/wk3_n), (wk4_wins/wk4_n), weekly_win_percentage),4)
...
indiv = data %>%
  select(`Name`,`Hour`, `Game 1`,`Game 2`, `Game 3`, `Game 4`, `Game 5`, `Game 6`, `Game 7`, `Game 8`, `Game 9`, `Game 10`, `Game 11`, `Game 12`, `Game 13`, `Game 14`, `Game 15`) %>%  janitor::clean_names()
...
indiv = indiv %>% 
  add_column(WK5_Correct = indiv_correct) %>%
  mutate(WK5_Percent = round(WK5_Correct/n,4)) %>%
  rename("Week_5" = WK5_Correct)%>%
  rename("Percent" = WK5_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name,hour, Week_5, Percent) %>%
  full_join(season) %>%
  ...
  mutate(Weeks_Picked = w1p+w2p+w3p+w4p+w5p) %>%
  mutate(games_picked = ((w1p*wk1_n)+(w2p*wk2_n)+(w3p*wk3_n)+(w4p*wk4_n)+(w5p*n))) %>%
  mutate(Season_Percent = round(((ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ...
                             ifelse(is.na(Week_5)==T,0,Week_5))/games_picked),4)) %>%
  select(name,hour, Week_1, Week_2, Week_3, Week_4, Week_5, Percent, Weeks_Picked, Season_Percent) %>%
...
Week_5_indiv = formattable(indiv_disp,
...
indi = indiv_disp %>% 
  select(name, Week_1, Week_2, Week_3, Week_4, Percent) %>%
