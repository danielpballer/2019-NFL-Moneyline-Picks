...
# week_12 = read_csv("NFL Week 12.csv") # Update
# week_13 = read_csv("NFL Week 13.csv") # Update
# week_14 = read_csv("NFL Week 14.csv") # Update
# data = read_csv("NFL Week 15.csv") # Update
...
week_7 = read_csv("../CSV_Data_Files/NFL Week 7 MA206.csv") 
week_8 = read_csv("../CSV_Data_Files/NFL Week 8 MA206.csv") 
week_9 = read_csv("../CSV_Data_Files/NFL Week 9 MA206.csv") 
week_10 = read_csv("../CSV_Data_Files/NFL Week 10 MA206.csv")
week_11 = read_csv("../CSV_Data_Files/NFL Week 11 MA206.csv")
week_12 = read_csv("../CSV_Data_Files/NFL Week 12 MA206.csv")
week_13 = read_csv("../CSV_Data_Files/NFL Week 13 MA206.csv") # Update
week_14 = read_csv("../CSV_Data_Files/NFL Week 14 MA206.csv") # Update
data = read_csv("../CSV_Data_Files/NFL Week 15 MA206.csv") # Update
```
...
wk_max = c(wk1_n, wk2_n, wk3_n, wk4_n, wk5_n, wk6_n, wk7_n, wk8_n, wk9_n, wk10_n, wk11_n, wk12_n, wk13_n, wk14_n, n)
...
wk11_wins = 12 #number of wins in week 11 
wk12_wins = 11 #number of wins in week 12
wk13_wins = 10 #number of wins in week 13 
wk14_wins = 9 #number of wins in week 14 #update this
...
wk14_combined_wins = 11 # number of combined wins in week 14 # update this

# who actually won
winners = c("Baltimore Ravens","New England Patriots","Tampa Bay Buccaneers","Houston Texans","Kansas City Chiefs","New York Giants","Philadelphia Eagles","Seattle Seahawks","Green Bay Packers","Minnesota Vikings","Oakland Raiders","Arizona Cardinals","Dallas Cowboys","Atlanta Falcons","Buffalo Bills","New Orleans Saints")

CBSsports = c("Baltimore Ravens","New England Patriots","Tampa Bay Buccaneers","Tennessee Titans","Kansas City Chiefs","Miami Dolphins","Philadelphia Eagles","Seattle Seahawks","Chicago Bears","Los Angeles Chargers","Oakland Raiders","Cleveland Browns","Los Angeles Rams","San Francisco 49ers","Buffalo Bills","New Orleans Saints")
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n+g9_n+g10_n+g11_n+g12_n+g13_n+g14_n #Update
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+wk11_wins+wk12_wins+wk13_wins+wk14_wins+correct_count$n[which(correct_count$Correct == "Yes")] #Update
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               ...
               (wk14_wins/g14_n), weekly_win_percentage),4) #Update
weeks = c("Week 1", "Week 2", "Week 3", "Week 4", "Week 5", "Week 6", "Week 7", "Week 8", "Week 9",
          "Week_10", "Week_11", "Week_12", "Week_13", "Week_14", "Week_15") #Update
...
season = indiv_weekly_pred(week_1, wk1_winners) %>%
  ...
  full_join(indiv_weekly_pred(data, winners) %>% ####### Add line for previous week and update to current week
      rename("Week_15" = Week)) %>%
  mutate(Percent = round(Week_15/n,4)) %>%
  arrange(-Percent)
...
```{r individual Display, eval=FALSE, include=FALSE}
...
indi = indiv_disp %>% 
  ...
  mutate(Week_14 = Week_14/(wk14_n))%>%#Update
  as.data.frame()
na = rep(indi$name, 15) # Update
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10,indi$Week_11,indi$Week_12,indi$Week_13,indi$Week_14, indi$Percent) # Update
we2 = c(rep("Week 1", length(indi$name)), rep("Week 2", length(indi$name)),
        ...
        rep("Week 11", length(indi$name)),rep("Week 12", length(indi$name)),
        rep("Week 13", length(indi$name)),rep("Week_14", length(indi$name)),
        rep("Week_14", length(indi$name))) #Update
