...
# data = read_csv("NFL Week 7.csv") # Update
week_1 = read_csv("../CSV_Data_Files/NFL Week 1 MA206.csv")
week_2 = read_csv("../CSV_Data_Files/NFL Week 2 MA206.csv")
week_3 = read_csv("../CSV_Data_Files/NFL Week 3 MA206.csv")
week_4 = read_csv("../CSV_Data_Files/NFL Week 4 MA206.csv")
week_5 = read_csv("../CSV_Data_Files/NFL Week 5 MA206.csv")
week_6 = read_csv("../CSV_Data_Files/NFL Week 6 MA206.csv")
data = read_csv("../CSV_Data_Files/NFL Week 7 MA206.csv") # Update
```
...
wk6_n = 13 # number of games in week 4
...
wk6_wins = 7 #number of wins in week 6 #update this
...
wk6_combined_wins = 7 # number of combined wins in week 6 # update this

# who actually won
winners = c("Kansas City Chiefs","Buffalo Bills","Jacksonville Jaguars","Minnesota Vikings","Green Bay Packers","Los Angeles Rams","Indianapolis Colts","San Francisco 49ers","Arizona Cardinals","Tennessee Titans","New Orleans Saints","Baltimore Ravens","Dallas Cowboys","New England Patriots")

CBSsports = c("Denver Broncos","Buffalo Bills", "Jacksonville Jaguars","Minnesota Vikings","Green Bay Packers","Los Angeles Rams","Houston Texans","San Francisco 49ers","Arizona Cardinals","Los Angeles Chargers","New Orleans Saints","Baltimore Ravens","Philadelphia Eagless","New England Patriots")
...
wk6_winners = c("New England Patriots","Carolina Panthers","Washington Redskins","Minnesota Vikings","Houston Texans","New Orleans Saints","Seattle Seahawks","San Francisco 49ers","Arizona Cardinals","New York Jets","Denver Broncos","Pittsburgh Steelers","Green Bay Packers") #update this
...
#wk5_CBSsports = c("Seattle Seahawks","Jacksonville Jaguars","New England Patriots","Tennessee Titans","Baltimore Ravens","Cincinnati Bangals","Houston Texans","Tamps Bay Buccaneers","Minnesota Vikings","Chicago Bears","Philadelphia Eagles","Los Angeles Chargers","Dallas Cowboys")
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+correct_count$n[which(correct_count$Correct == "Yes")]
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               (wk6_wins/g6_n), weekly_win_percentage),4)
weeks = c("Week 1", "Week 2", "Week 3", "Week 4", "Week 5", "Week 6", "Week 7")
...
games_wk6 = week_6 %>% janitor::clean_names() %>%
  select(starts_with("game"))  #Creating the set of just games
...
indiv = data %>%
  select(`Name`,`Hour`, `Game 1`,`Game 2`, `Game 3`, `Game 4`, `Game 5`, `Game 6`, `Game 7`, `Game 8`, `Game 9`, `Game 10`, `Game 11`, `Game 12`, `Game 13`, `Game 14`) %>%  janitor::clean_names()
...
indiv = indiv %>% 
  add_column(WK7_Correct = indiv_correct) %>%
  mutate(WK7_Percent = round(WK7_Correct/n,4)) %>%
  rename("Week_7" = WK7_Correct)%>%
  rename("Percent" = WK7_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name, hour, Week_7, Percent) %>%
  full_join(season) %>%
  ...
  mutate(Weeks_Picked = w1p+w2p+w3p+w4p+w5p+w6p+w7p) %>%
  ...
  mutate(Season_Percent = round(((ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ...
                             ifelse(is.na(Week_7)==T,0,Week_7))/games_picked),4)) %>%
  select(name, hour, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Week_7, 
...
Week_7_indiv = formattable(indiv_disp,
...
indi = indiv_disp %>% 
  select(name, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Percent) %>%
  ...
  as.data.frame()
na = rep(indi$name, 7)
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Percent)
