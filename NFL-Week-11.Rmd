...
data = read_csv("NFL Week 9.csv") # Update
...
cadet_wk11 = read_csv("NFL Week 9 MA206.csv") # Update
```
...
wk10_wins = 7 #number of wins in week 10 #update this
...
wk10_combined_wins = 7 # number of combined wins in week 10 # update this

# who actually won
winners = c("San Francisco 49ers","Houston Texans", "Philadelphia Eagles","Pittsburgh Steelers","Miami Dolphins","Kansas City Chiefs","Carolina Panthers","Buffalo Bills","Seattle Seahawks","Oakland Raiders","Los Angeles Chargers","Denver Broncos","Baltimore Ravens","Dallas Cowboys") 

CBSsports = c("San Francisco 49ers","Houston Texans", "Philadelphia Eagles","Indianapolis Colts","New York Jets","Minnesota Vikings","Carolina Panthers","Buffalo Bills","Seattle Seahawks","Oakland Raiders","Green Bay Packers","Cleveland Browns","Baltimore Ravens","Dallas Cowboys")
...
wk10_winners = c("Oakland Raiders","Baltimore Ravens","Cleveland Browns","Chicago Bears","New York Jets","Tennessee Titans","Tampa Bay Buccaneers","Atlanta Falcons","Indianapolis Colts","Los Angeles Rams","Green Bay Packers","Minnesota Vikings","Seattle Seahawks") # Update this 
...
season_games = group_games+g1_n+g2_n+g3_n+g4_n+g5_n+g6_n+g7_n+g8_n+g9_n+g10_n
season_wins  = wk1_wins+wk2_wins+wk3_wins+wk4_wins+wk5_wins+wk6_wins+wk7_wins+wk8_wins+wk9_wins+
  wk10_wins+correct_count$n[which(correct_count$Correct == "Yes")]
...
wins = round(c((wk1_wins/g1_n),(wk2_wins/g2_n),(wk3_wins/g3_n), (wk4_wins/g4_n), (wk5_wins/g5_n),
               (wk6_wins/g6_n),(wk7_wins/g7_n),(wk8_wins/g8_n), (wk9_wins/g9_n), (wk10_wins/g10_n),
               weekly_win_percentage),4)
...
# Calculating Week 10 Individual Results
indiv_wk10 = week_10 %>% janitor::clean_names() %>%
  select(name, starts_with("game"))

games_wk10 = indiv_wk10 %>%
  select(-name)  #Creating the set of just games

indiv_correct_wk10 = NULL
help_wk10 = NULL
for (i in 1:length(indiv_wk10$name)){
  for(j in 1:length(games_wk10)){
    help_wk10[j] = ifelse(games_wk10[i,j]==wk10_winners[j],1,0)
  indiv_correct_wk10[i] = sum(help_wk10)
  }
}

# Calculating individual win number for week 10
season_wk10 = indiv_wk10 %>%
  select(name) %>%
  add_column(Week_10 = indiv_correct_wk10)

# Adding week 10 to week 1-9
season = season %>%
  full_join(season_wk10)
```
...
indiv = indiv %>% 
  add_column(WK11_Correct = indiv_correct) %>%
  mutate(WK11_Percent = round(WK11_Correct/n,4)) %>%
  rename("Week_11" = WK11_Correct)%>%
  rename("Percent" = WK11_Percent)%>%
  arrange(-Percent)

# choosing just name # correct pics and correct percentage
indiv_disp = indiv %>%
  select(name, Week_11, Percent) %>%
  full_join(season) %>%
  ...
  mutate(w11p = ifelse(is.na(Week_11)==T,0,1)) %>%
  mutate(Weeks_Picked = w1p+w2p+w3p+w4p+w5p+w6p+w7p+w8p+w9p+w10p+w11p) %>%
  mutate(games_picked = ((w1p*wk1_n)+(w2p*wk2_n)+(w3p*wk3_n)+(w4p*wk4_n)+(w5p*wk5_n)+
                           (w6p*wk6_n)+(w7p*wk7_n)+(w8p*wk8_n)+(w9p*wk9_n)+(w10p*wk10_n)+
                           (w11p*n))) %>%
  mutate(Season_Percent = round(((ifelse(is.na(Week_1)==T, 0, Week_1) + 
                             ...
                             ifelse(is.na(Week_10)==T,0,Week_10)+
                             ifelse(is.na(Week_11)==T,0,Week_11))/games_picked),4)) %>%
  select(name, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Week_7, Week_8, Week_9, Week_10, 
         Week_11, Percent, Weeks_Picked, Season_Percent) %>%
...
Week_11_indiv = formattable(indiv_disp,
...
indi = indiv_disp %>% 
  select(name, Week_1, Week_2, Week_3, Week_4, Week_5, Week_6, Week_7, Week_8, Week_9, Week_10, Percent) %>%
  ...
  as.data.frame()
na = rep(indi$name, 11)
we1 = c(indi$Week_1,indi$Week_2,indi$Week_3,indi$Week_4, indi$Week_5, indi$Week_6, indi$Week_7, indi$Week_8, indi$Week_9,indi$Week_10, indi$Percent)
...
c_games = cadet_wk11%>% janitor::clean_names() %>%
  select(starts_with("game"))  #Creating the set of just games
...
c_wins = round(c((wk1_combined_wins/wk1_n), (wk2_combined_wins/wk2_n),(wk3_combined_wins/wk3_n),
           ...
           (wk10_combined_wins/wk10_n), t_weekly_win_percentage), 4)
